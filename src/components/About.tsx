    ...
    <section id="about" className="relative py-24 md:py-32 bg-background">
      <div className="max-w-7xl mx-auto px-6 md:px-24">
        ...
        <div className="grid md:grid-cols-12 gap-12 md:gap-24 relative">
          ...
          >
            <p className="text-xl md:text-3xl leading-relaxed font-light text-text-primary">
            ...
            <p className="text-text-secondary leading-relaxed text-lg pb-12 editorial-border-b">
              I believe in writing clean, purposeful code. When I&apos;m not optimizing algorithms or contributing to open source, you&apos;ll find me experimenting with new AI technologies and creating tools that solve real-world problems.
            </p>
            ...
            <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-3">
              {[
                ...
                <div key={stat.label} className="border border-border bg-surface/70 px-5 py-6 text-center md:text-left">
                  <span className="block font-heading text-3xl md:text-4xl font-black text-accent">{stat.value}</span>
                  <span className="mt-2 block font-mono text-[10px] md:text-xs text-text-secondary uppercase tracking-widest">{stat.label}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          >
            {highlights.map((item, index) => (
              ...
              >
                <div className="absolute left-1/2 -translate-x-1/2 sm:-translate-x-0 sm:-left-4 -top-6 text-[8rem] sm:text-[6rem] font-heading font-black text-text-primary/[0.03] select-none pointer-events-none group-hover:text-accent/5 transition-colors duration-500">
                  0{index + 1}
                </div>

                <div className="font-mono text-base font-bold text-accent pt-1 relative z-10 border border-accent/40 bg-accent/5 px-3 py-2">
                  {item.icon}
                </div>
